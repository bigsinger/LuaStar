...
`zip` 的 PowerShell 压缩进度不受调试开关影响：交互式控制台中始终显示，输出被重定向或在无人值守环境中运行时自动关闭，避免把进度控制信息混入日志和返回文本。
...
在交互式控制台中，PowerShell 会实时显示 `Compress-Archive` 的处理进度。该进度仅用于观察，不写入第二个返回值；输出重定向到文件或管道时会自动关闭。
